...
interface DailyLogDao {
    ...
    @Query("SELECT * FROM daily_logs WHERE date = :date LIMIT 1")
    suspend fun getLogForDateOnce(date: LocalDate): DailyLog?
    ...
    @Query("SELECT * FROM daily_logs WHERE bleeding_intensity IS NOT NULL AND bleeding_intensity > 0 ORDER BY date ASC")
    suspend fun getAllBleedingLogs(): List<DailyLog>
